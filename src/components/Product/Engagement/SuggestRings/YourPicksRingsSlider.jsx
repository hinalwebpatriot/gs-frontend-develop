...
import { fetchEngagementRingYourPicks } from "../EngagementPageActions";

class YourPicksRingsSlider extends React.Component {
  constructor(props) {
    super(props);
    ...
    return (
      <section className="recently-section ">
        <div className="container">
          ...
          {/*<p className="theme-subtitle theme-subtitle--type2  theme-subtitle--smaller">{data.subtitle}</p>*/}
          {/*</div>*/}
          {/*</div>*/}
...
const mapStateToProps = (state, props) => {
  const { data, status } = selectors.youHaveSeenRings(state);
  return { data, status, ...props };
};

const mapDispatchToProps = {
  fetchData: fetchEngagementRingYourPicks
};

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(YourPicksRingsSlider);
