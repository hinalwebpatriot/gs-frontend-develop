...
export default function generateRemoveFromCart({
  products = [],
  quantity = 1
}) {
  const currencyCode = get(products, "[0].price.currency", "AUD");

  const data = products.map(item => generateProductData(item, { quantity }));

  const event = {
    category: GoogleEE.EVENT_CATEGORY,
    action: GoogleEE.EVENT_ACTION_REMOVE_FROM_CART
  };
