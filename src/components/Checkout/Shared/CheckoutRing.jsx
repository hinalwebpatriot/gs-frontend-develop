...
const CartRing = ({ data, showActions, list, position, currentMethodName }) => {
  const { preview_image, image, sku, title, product_type, id, h1 } = data;

  let link;

  switch (product_type) {
    case 'engagement-rings':
      link = routing({ id, slug: h1 }).engagementProduct;
      break;
    case 'wedding-rings':
      link = routing({ id, slug: h1 }).weddingProduct;
      break;
    default:
      break;
  }
      ...
      <div className="slide cart-product__info">
        ...
        {product_type === 'wedding-rings' && (
          <div className="cart-prod-option">
            <p>Gender: </p>
            <span>{get(data, 'options.gender', '')}</span>
          </div>
        )}
        <div className="cart-prod-option">
          <p>Metal: </p>
        ...
        <div className="cart-prod-option">
          <p>Ring size: </p>
          <span>{get(data, 'selected_size.title.au', '')}</span>
        </div>
        <div className="cart-prod-option">
          <p>Engraving: </p>
        ...
        <div className="cart-prod-total">
          ...
          <p className="table-price table-price--type3 " style={{ color: '#000000' }}>
            {localPrice}
            <span>{!isServer && localeStore.taxString.split('.').join('. ')}</span>
          </p>
