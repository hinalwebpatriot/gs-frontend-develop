...
export const filterChange = createRoutine("CATALOG_FILTER_CHANGED");

export const setCatalogCategory = createRoutine("SET_CATALOG_CATEGORY");
export const setLastCatalogCategory = createRoutine("SET_LAST_CATALOG_CATEGORY");
...
export const changeCatalogGenderTab = createRoutine("CATALOG_GENDER_CHANGE");
//
// export const setCatalogShape = createRoutine('CATALOG_SHAPE_SET');
...
function* fetchFeedWorker({ payload = {} }) {
  ...
  try {
    ...
    yield put(setLastCatalogCategory.trigger(payload.slug === 'rings' ? 'cluster-rings' : payload.slug));
    yield put(setMetaImage(get(res, 'data.data[0].products[0].preview_image.path.origin', '')));
  } catch (e) {
...
export function* catalogFeedServerWorker({ settings, seo, feed }) {
  ...
  yield fork(fetchSeoTextBlockWorker, { payload: `jewellery-${metaSlug}` });
  yield fork(fetchMetaWorker, { payload: `jewellery-${metaSlug}` });
...
function* filterChangedWorker({ payload }) {
  if (payload.filter) {
    payload.history.push(routing({ catalog: payload.catalog, filter: payload.filter}).catalogFeedWithFilter)
  } else {
    payload.history.push(routing(payload.catalog).catalogFeed)
  }
}
